...
import {
  createMasterSession,
  recordMasterLoginAttempt,
  validateMasterLoginAllowed,
  writeMasterAction,
} from '@/lib/app';
...
export async function POST(request: Request) {
  ...
  if (!gate.ok) {
    ...
    const errorMessage = gate.message ?? 'Login failed';
    return NextResponse.redirect(
      new URL(`/master/login?error=${encodeURIComponent(errorMessage)}`, origin)
    );
  }
  ...
  if (!valid) {
    return NextResponse.redirect(
      new URL('/master/login?error=Invalid%20login', origin)
    );
  }
